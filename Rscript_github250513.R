...
lf <- list.files(full.names = T) %>% str_subset(".csv")  
lf2 <- lf %>% str_subset("84FM|85FM|86FM|87FM")
#

FMsp <- function(x){ # x:matrix
  
  coln <- colnames(x)
  Tsp <- length(coln) #オスとメスを分けたときの総種数
  Fsp <- length(coln[str_detect(coln, "FA")]) # メスの種数
  Msp <- length(coln[str_detect(coln, "MA")]) # オスの種数
  
  spnum <- coln %>% str_replace("FA", "") %>% str_replace("MA", "")
  spnum <- type.convert(spnum) # as.integerでうまくいかないのでこちらで数値に変換
  
  Ssp <- length(table(spnum)) #オスとメスが両方いる種をまとめた合計種数
  
  Bothsp <- Tsp - Ssp #オスとメスが両方いる種の数
  Fosp <- Fsp - Bothsp #メスのみ出現する種数
  Mosp <- Msp - Bothsp #オスのみ出現する種数
  
  return(c(Fosp, Bothsp, Mosp))
  
}

dat2 <- sapply(dat, FMsp) #メス、両方、オス　の種数
colnames(dat2) <- c("Ashu84", "Ashu85", "Ashu86", "Ashu87", "Kibune84", "Kibune85", "Kibune86", "Kibune87", "KyotoU85", "KyotoU86", "KyotoU87")
rownames(dat2) <- c("Female", "Both", "Male")
dat3 <- as.data.frame(dat2)
dat3 <- rownames_to_column(dat3, var="Sex")
dat3 <- gather(dat3, key=Network, value=Sp, -Sex) # データのまとめ
dat3

# ratio data
dat4 <- t(dat2)/apply(t(dat2), 1, sum) #割り算は行方向に実行される
dat4 <- t(dat4)
dat4 <- as.data.frame(dat4)
dat4 <- rownames_to_column(dat4, var="Sex")
dat4 <- gather(dat4, key=Network, value=Sp, -Sex) # データのまとめ

dat4$Sex2 <- factor(dat4$Sex, levels=c("Female", "Both", "Male")) # 積む順番を変える


# make plots

dat4$Sex2 %>% unique() # Female Both Male

xlabels <- c("Ashu84" = "芦生84", "Ashu85" = "芦生85", "Ashu86" = "芦生86", "Ashu87" = "芦生87",
             "Kibune84" = "貴船84", "Kibune85" = "貴船85", "Kibune86" = "貴船86", "Kibune87" = "貴船87",
             "KyotoU85" = "京大85", "KyotoU86" = "京大86", "KyotoU87" = "京大87")

p <- ggplot(data=dat4, aes(x=Network, y=Sp, fill=Sex2))+
  theme_bw()+
  theme(
    panel.grid.minor = element_blank(),
    panel.grid.major = element_blank(),
    legend.position = "none",
    axis.text.x = element_text(size = 20, angle = 90, hjust = 1, colour="black"),
    axis.text.y = element_text(size = 20, colour="black"),
    #text = element_text(size=30),
    axis.title.x = element_blank(),
    axis.title.y = element_text(size = 20, color = "black"),
    plot.margin = unit(c(5, 5, 10, 5), units="mm")
    )+
  ylab("Ratio")+
  geom_bar(stat="identity", position="fill", color="black", width = 0.7)+ #x、y両方に値を指定する場合、stat="identity"
  scale_y_continuous(labels = percent, expand = c(0,0))+
  scale_fill_manual(values = c("tomato", "black", "olivedrab3"))+
  scale_x_discrete(labels = xlabels)+
  ylab("割合（%）")
p

## save
ggsave(p, filename = "jbpress/FMratio250318.jpg", units = "cm", width = 20, height = 13.4, dpi = 300, bg = "white")

##


datF[[1]]
net_names <- names(xlabels)
...
datF[[1]] %>% as.vector() %>% 
  ...
  set_colnames("obs") %>% 
  filter(obs != 0) %>% 
  mutate(net = net_names[1])

freq_F_df <- 1:11 %>% map(\(x){
  res <- datF[[x]] %>% 
    as.vector() %>% 
    as_tibble() %>% 
    drop_na() %>% 
...
p_F <- freq_F_df %>% 
  ggplot(aes(x = log(rank), y = log(obs), group = net))+
  theme_cowplot()+
  geom_line()+
  xlim(c(0, 6))+
  ylim(c(0, 5))
...
p_F + p_M + plot_layout(ncol = 2)

 
p <- freq_F_df %>% 
  mutate(sex = "F") %>% 
  rbind(freq_M_df %>% mutate(sex = "M")) %>% 
  ggplot(aes(x = log(rank), y = log(obs), fill = net, color = sex))+
  theme_cowplot()+
  geom_point()+
  geom_line()+
#  geom_smooth(method = "lm", se = F)
  scale_color_manual(values = c("F" = "tomato", "M" = "darkgreen"),
                     labels = c("F" = "メスのネットワーク", "M" = "オスのネットワーク"))+
  guides(fill = "none")+
  theme(
  ...
  labs(x = "log(順位)", y = "log(観察回数)")
p
ggsave(p, filename = "loglog_freescale250318.jpeg", units = "cm", width = 13.4, height = 13.4, dpi = 300, bg = "white")
...
se_res2 <- second.extinct(datF[[1]], method = "random", nrep = 100)
...
se_df_F %>% 
  ...
  rbind(se_df_A %>% mutate(sex = "A")) %>% 
  filter(net == "Kibune85") %>% 
  ggplot(aes(no, cm_lower_ratio, color = sex))+
  theme_cowplot()+
  geom_line()
...
p <- se_df_F %>% 
  ...
  labs(x = "取り除く昆虫の割合", y = "絶滅する花の割合", title = "芦生, 86年")+
  theme(
    legend.title = element_blank(),
    legend.position = "inside",
  ...
  scale_color_manual(values = c("A" = "black", "F" = "tomato", "M" = "green3"), 
                     labels = c("A" = "全体", "F" = "メスのネットワーク", "M" = "オスのネットワーク"))
p
## save
ggsave(p, filename = "jbpress/ashu86_secondextinct.jpeg", 
       units = "cm", height = 13.4, width = 13.4, dpi = 300, bg = "white")
...
p <- se_df_F %>% 
  ...
  geom_ribbon(aes(x = no, ymax = cm_lower_ratio, ymin = 1-no, fill = sex))+
  facet_wrap(sex~., nrow = 1, labeller = as_labeller(c("A" = "全体", "F" = "メス", "M"= "オス")))+
  #scale_color_manual(values = c("A" = "black", "F" = "tomato", "M" = "green3"))+
  scale_fill_manual(values = c("A" = "black", "F" = "tomato", "M" = "green3"))
p
p %>% ggsave(filename = "jbpress/ashu86_plotarea_wrap.jpeg", 
             units = "cm", height = 13.4, width = 20, dpi = 300, bg = "white")
...
p <- lorenz_area %>% 
  ...
  labs(y = "カーブと直線に囲まれた面積")+
  scale_x_discrete(labels = c("A" = "全体", "F" = "メス", "M" = "オス"))+
...
p %>% ggsave(filename = "jbpress/lorenz_area250318.jpeg", units = "cm", 
             height = 13.4, width = 20, dpi = 300, bg = "white")
...
datF[[1]] %>% 
  second.extinct(method = "random", nrep = 100) %>% 
  asplit(1) %>% map(\(x)x %>% t() %>% as.data.frame()) %>% list_rbind() %>% 
  mutate(net = net_names[1]) %>% # 芦生とか
  mutate(trophic = "upper") %>% # .$ext.lower %>% sum()
  mutate(cm_lower = row_num - cumsum(ext.lower)) %>% 
  mutate(cm_lower_ratio = cm_lower/row_num) %>% 
  mutate(no = no/nrow(.)) %>% 
  ggplot(aes(no, cm_lower_ratio))+
  geom_line()
...
datF[[1]] %>% 
  second.extinct(participant = "higher", method = "random", nrep = 100) %>% 
  head()
...
#* オスもメスもネットワークの頑健性は変わらなかった。
#* 昆虫を抜いていく場合にも花を抜いていく場合でも変わらん。
#
...
p %>% ggsave(filename = "lorenz_areaplot250411.jpeg", units = "cm", 
...
p <- se_df_F %>% 
  ...
  #scale_fill_manual(values = c("A" = "black", "F" = "tomato", "M" = "green3"))
p
p %>% ggsave(filename = "ashu86_plotarea_wrap250411.jpeg", 
...
p %>% ggsave(filename = "lorenz_area_flower_plot250415.jpeg", units = "cm", 
             height = 10, width = 15, dpi = 300, bg = "white")
...
p_2 %>% 
  ggsave(filename = "ashu86_areacomparison_wrap250418.jpeg", 
